...
<Steps>
  <Step title='Say "start recording"'>
    ...
    navigation, and mutating xhr/fetch is captured to a JSON event file. Clicks also
    store x/y and a highlighted screenshot.
  </Step>
...
- **The agent sees where you clicked.** Each click stores page x/y and a screenshot
  with a rectangle on the target, plus mutating xhr/fetch, so the generated skill
  knows the **expected outcome** of every step.
...
7   7.1   screenshot          /tmp/playwriter-recorder/1/6.png
8   7.3   network             POST 200 https://directory.example.com/api/products
```
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `action`                      | Click, fill, press, select — with generated Playwright locator code, and `x`/`y` on clicks                                                                                |
| `screenshot`                  | Viewport PNG after a click, with a rectangle on the clicked box. `path` + `afterActionId`                                                                                 |
...
| `scroll` / `signal`           | Window scroll and navigation signals                                                                                                                                      |

Screenshot events carry `afterActionId`, linking the image to the click that caused it:
...
**`SKILL.md`** — the high-level flow: preconditions (already signed-in browser),
numbered steps with locators, the expected outcome of each step (url change,
confirmation text, mutating network request), and how to detect success.
