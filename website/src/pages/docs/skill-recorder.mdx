...
description: Record a browser workflow once by hand, and let your agent turn it into a reusable skill with verified Playwright locators, expected outcomes, and importable utils.
...
writes a **skill**: a SKILL.md plus a utils script that automates the same flow forever.

<Steps>
  <Step title='Say "start recording"'>
    Your agent runs `playwriter recorder start`. From this moment every click, fill,
    navigation, network request, cookie/storage change, and snapshot diff is captured
    to a jsonl file.
  </Step>
  ...
  <Step title='Say "done"'>
    The agent runs `playwriter recorder stop` and reads the recorded events, verifying
    each locator against the live page.
  </Step>
  <Step title='The agent writes the skill'>
    It produces a `SKILL.md` plus a `utils.js` with importable functions, then validates
    the whole flow by replaying it end-to-end.
  </Step>
</Steps>
...
- **The agent sees what changed, not just what you clicked.** Each action is linked to
  the aria snapshot diff, network requests, and cookie/storage changes it caused, so the
  generated skill knows the **expected outcome** of every step.
...
# Events file: ~/.playwriter/recordings/1.jsonl
...
(response bodies, snapshot diffs) show up as sizes, so reading the whole recording costs
few tokens:

```bash
playwriter recorder events 1 | jq -r '[.id, .t, .type, (.code // .url // empty)] | @tsv'
```
...
8   7.6   snapshot-diff       + text: "Thanks! Your product is under review."
```

Then it drills into the events that matter, by id:

```bash
playwriter recorder events 1 7   # full request postData + responseBody of event 7
```
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `action`                      | Click, fill, press, select — with generated Playwright locator code and an `actionId`                                                                                     |
| `snapshot-diff`               | How the page's aria snapshot changed after each action                                                                                                                    |
| `network`                     | Document/XHR/fetch requests: method, url, status, post data, and **response bodies** for JSON/text responses (shown as sizes in the thin view, full content per event id) |
| `download`                    | File downloads: source url and suggested filename                                                                                                                         |
| `file-upload`                 | Files chosen in `input[type=file]` elements (file names)                                                                                                                  |
| `cookies` / `storage`         | Cookie and localStorage/sessionStorage changes (values hashed or truncated)                                                                                               |
| `navigation` / `url-changed`  | Full navigations and SPA pushState url changes                                                                                                                            |
...
| `focus` / `scroll` / `signal` | Focus changes, scrolling, navigation signals                                                                                                                              |

State-change events carry `afterActionId`, linking each UI/network/storage change to the
action that caused it. That's what lets the agent write skills with **verifiable expected
outcomes** for every step:

```bash
# everything action 3 caused
playwriter recorder events 1 | jq 'select(.afterActionId == 3)'
```

## What the agent produces

The `recorder start` output includes full instructions for the agent: ask you to summarize
what you did, inspect the events, verify each locator against the live page, ask for the
skill name and which values should become parameters, then write two files:

**`SKILL.md`** — the high-level flow: preconditions (login state from the recorded cookie
events), numbered steps with locators, the expected outcome of each step (url change,
snapshot text, network request), and how to detect success.

**`utils.js`** — importable functions that replay the flow with few tokens:
...
export async function submitProduct({ page, name, url }) {
  ...
  await page.getByText('Thanks! Your product is under review.').waitFor()
}
```

Any agent can then run the whole flow in one command:

```bash
playwriter -s 1 -e "const { submitProduct } = await import('./skills/submit-to-directory/utils.js'); await submitProduct({ page, name: 'Acme', url: 'https://acme.com' })"
```

The agent validates the skill by replaying it end-to-end before calling it done.
...
playwriter recorder events 1 | jq -r 'select(.type == "network" and .responseBodySize) | [.id, .method, .url, .responseBodySize] | @tsv'
# then read the full request/response payloads of the interesting endpoints
playwriter recorder events 1 14 15 22
```
...
in words; the recording captures the exact locators (with iframe paths) and the snapshot
diffs that confirm each step landed. The skill becomes `createLead({ name, email, owner })`
that any agent calls in one command.
...
name and dates, and calls `createBooking({ guest, checkIn, checkOut, roomType })` —
...
produced it, so the agent writes `downloadInvoices({ month })` per vendor. The same pattern
...
is filled, visible in the snapshot diffs.

### E2E tests from a manual QA pass

A QA person clicks through a critical flow once; the agent turns the recording into a
Playwright test. Better than codegen alone, because the recording also contains the
**expected outcomes** — aria diffs become assertions, network status codes become response
checks.
...
against the old utils file and updates only the selectors that changed, keeping the
...
- Events persist in `~/.playwriter/recordings/<id>.jsonl` (readable only by your user).
...
<Aside>
<Tip>
The best prompt is simple: **"start recording, I'll show you the workflow"**. Everything
else — questions about skill name, parameters, and location — the agent asks at the right
time, guided by the instructions `recorder start` prints.
</Tip>
</Aside>
